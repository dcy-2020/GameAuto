...
- **看门狗循环重构**：三个游戏的重复看门狗逻辑合并为通用 `_run_with_watchdog()` 方法，消除 ~300 行重复代码
- **性能优化**：循环内配置项缓存为局部变量，减少字典查询开销
- **异常处理加固**：12 处裸 `except:` 改为 `except Exception:`，防止误捕 `KeyboardInterrupt`
- **全局超时可配置**：新增 `max_program_runtime` 配置项，用户可在 GUI 中调整程序总运行时间上限
- **导入简化**：消除 `__import__()` 运行时反射导入，改为标准顶部 import
- **清理逻辑统一**：鸣潮模块成功/失败分支的清理代码合并为一处

### v2.0

- 游戏日常自动化工具 —— 鸣潮、终末地、异环三款游戏编排调度
- 图形界面配置，5 套主题皮肤，AI 异常处理，钉钉推送
